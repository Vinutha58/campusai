...
SYSTEM_PROMPT = (
    "You are CampusGPT, a helpful academic assistant for college students on the CampusAI "
    "platform. Help with doubts across subjects, summarizing notes, building study plans, and "
    "general academic guidance. Be concise, clear, and encouraging. You do not have access to "
    "the student's actual courses, grades, or materials, so ask clarifying questions when you "
    "need specifics."
)
...
async def list_messages(current_user: UserPublic = STUDENT_ONLY):
    db = get_database()
    messages = (
        await db.campusgpt_messages.find({"user_id": ObjectId(current_user.id)})
        .sort("created_at", 1)
        .to_list(length=500)
    )
...
async def send_message(data: ChatMessageCreate, current_user: UserPublic = STUDENT_ONLY):
    ...
    result = await db.campusgpt_messages.insert_one(user_doc)
    ...
    result = await db.campusgpt_messages.insert_one(assistant_doc)
...
async def clear_history(current_user: UserPublic = STUDENT_ONLY):
    db = get_database()
    await db.campusgpt_messages.delete_many({"user_id": ObjectId(current_user.id)})
