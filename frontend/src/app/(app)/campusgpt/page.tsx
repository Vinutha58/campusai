...
import { Bot, Send, Trash, TriangleAlert } from "lucide-react";
...
function CampusGPTContent() {
  ...
  const handleSend = async (e: FormEvent) => {
    ...
    setSending(true);
    try {
      const exchange = await sendChatMessage(content);
    ...
    <div className="mx-auto flex h-full w-full max-w-3xl flex-col gap-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">CampusGPT</h1>
          ...
          {loaded && messages.length === 0 && (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Ask CampusGPT about a doubt, a topic to study, or a plan for your week.
              </p>
            </div>
          )}
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
                ...
                >
                  <p>{m.content}</p>
                  <p className={`mt-1 text-[10px] ${isUser ? "text-indigo-100" : "text-zinc-400"}`}>
...
export default function CampusGPTPage() {
  ...
  if (user.role === "student") return <CampusGPTContent />;
  return <ComingSoon title="CampusGPT" />;
}
